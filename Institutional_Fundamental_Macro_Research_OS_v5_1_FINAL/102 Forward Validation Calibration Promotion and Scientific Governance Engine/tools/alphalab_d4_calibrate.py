...
def cluster_boot(rows,value_field,preferred='independent_root_id',fallback='trading_day',it=2000,seed=17):
    ...
    if key:
        ...
        # Equal weight by independent cluster: first summarize each cluster, then resample cluster means.
        units=[mean(vs) for vs in g.values()]
        unit=key
    else:
        units=[v for _,v in vals];unit='OBSERVATION_FALLBACK'
    ...
    bs.sort();lo=bs[int(.025*(it-1))];hi=bs[int(.975*(it-1))]
    return {'interval':[lo,hi],'unit':unit,'cluster_n':len(units),'observation_n':len(vals)}
...
def part(label):
    ...
    boot=cluster_boot(realized,'delta_r',pref,fallback,it)
    regime=group_mean(realized,'delta_r','regime')
    signs=[1 if v['mean']>0 else -1 if v['mean']<0 else 0 for v in regime.values() if v['mean'] is not None]
    stable=None if not signs else (all(s>=0 for s in signs) or all(s<=0 for s in signs))
    tail_actual=tail(actual);tail_cf=tail(cf);tail_delta=None if tail_actual is None or tail_cf is None else tail_actual-tail_cf
    censored=len(rr)-len(realized)
...
dev=part('DEVELOPMENT');ho=part('HOLDOUT')
hf=sorted(set(r.get('hypothesis_family_id') for r in rows if r.get('hypothesis_family_id')))
mt_counts=[]
for r in rows:
    try:
        if r.get('hypotheses_tested_in_family'):mt_counts.append(int(r['hypotheses_tested_in_family']))
    except:pass
out={'record_type':'CALIBRATION_REPORT','calibration_version':'1.1.0','modifier_id':a.modifier_id,'hypothesis_family_ids':hf,'development':dev,'holdout':ho,'dependence':{'preferred_cluster_key':pref,'fallback_cluster_key':fallback,'development_resampling_unit':dev['resampling_unit'],'holdout_resampling_unit':ho['resampling_unit'],'observation_level_bootstrap_used':dev['resampling_unit']=='OBSERVATION_FALLBACK' or ho['resampling_unit']=='OBSERVATION_FALLBACK'},'tail_risk':{'development_lower_tail_delta_r_5pct':dev['lower_tail_delta_r_5pct'],'holdout_lower_tail_delta_r_5pct':ho['lower_tail_delta_r_5pct'],'comparison_available':ho['lower_tail_delta_r_5pct'] is not None},'regime_stability':{'development':dev['regime_effects'],'holdout':ho['regime_effects'],'holdout_sign_stable':ho['effect_sign_stable_across_observed_regimes']},'multiple_testing':{'hypothesis_family_ids':hf,'max_declared_hypotheses_tested':max(mt_counts) if mt_counts else None,'disclosure_complete':bool(hf) and bool(mt_counts)},'chronological_split_attested':all((r.get('chronological_split_attested') or '').lower() in {'true','1','yes'} for r in rows) if rows else False,'execution_profiles':sorted(set(r.get('execution_profile') for r in rows if r.get('execution_profile'))),'note':'Dependence-aware descriptive policy-effect calibration. Independent-root/trading-day cluster resampling is preferred; promotion still requires holdout, tail, multiplicity, stability and validator governance.'}
