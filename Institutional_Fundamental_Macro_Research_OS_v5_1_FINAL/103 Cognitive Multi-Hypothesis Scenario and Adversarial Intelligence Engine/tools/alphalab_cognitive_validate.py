...
p=argparse.ArgumentParser();p.add_argument('--input',required=True);p.add_argument('--schema',default=None);p.add_argument('--vault-root');p.add_argument('--evidence-pack');p.add_argument('--require-evidence-pack',action='store_true');p.add_argument('--d4-ledger');a=p.parse_args();mod=Path(__file__).resolve().parent.parent;schema=Path(a.schema) if a.schema else mod/'schemas/AlphaLab_Cognitive_Input.schema.json'
