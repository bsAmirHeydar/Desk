...
def main():
 ...
 ok=all(x['status']=='PASS' for x in checks);out={'phase':'AD-V3-P12','version':'3.12.1-v31-r01-qualified','acceptance_status':'PASS' if ok else 'FAIL_CLOSED','check_count':len(checks),'checks':checks,'phase_status':phase_status,'golden_status':gold.get('status'),'real_p09_forward_state':stats.get('forward_evidence_state'),'real_p09_mature_episodes':stats.get('mature_episode_count'),'certification_state':st,'production_promotion_performed':resolve(REPO,'Gold','PRODUCTION')['selected_runtime']=='V3','trade_execution_authority':'NONE'};print(json.dumps(out,ensure_ascii=False,indent=2));return 0 if ok else 2
