...
def main():
 ...
 checks.append(ck('every final gate independently blocks promotion',all(blocked.values()),blocked));checks.append(ck('approval without P09 denied',blocked.get('P09_FORWARD_EVIDENCE_PROVISIONAL') is True));checks.append(ck('gates without approval denied',blocked.get('EXPLICIT_OPERATOR_APPROVAL') is True))
 # Real P09 truth and canonical source.
 p09=real_p09();stats=p09.get('statistics') or {};checks.append(ck('real P09 state read canonically',stats.get('fixture_samples_included') is False and stats.get('replay_samples_included') is False,stats));checks.append(ck('legacy/replay/fixture cannot satisfy promotion by construction','P09_FORWARD_EVIDENCE_PROVISIONAL' in GATE_IDS and (p09.get('legacy') is not None)))
 ...
 st=certification_state(REPO);checks.append(ck('certification state truthfully fail-closed',st in ('CERTIFIED_PENDING_FORWARD_EVIDENCE','PRODUCTION_ELIGIBLE','PRODUCTION_V3','ENGINEERING_CERTIFIED'),st))
 ok=all(x['status']=='PASS' for x in checks);out={'phase':'AD-V3-P12','version':'3.12.0-final-certification-freeze','acceptance_status':'PASS' if ok else 'FAIL_CLOSED','check_count':len(checks),'checks':checks,'phase_status':phase_status,'golden_status':gold.get('status'),'real_p09_forward_state':stats.get('forward_evidence_state'),'real_p09_mature_episodes':stats.get('mature_episode_count'),'certification_state':st,'production_promotion_performed':resolve(REPO,'Gold','PRODUCTION')['selected_runtime']=='V3','trade_execution_authority':'NONE'};print(json.dumps(out,ensure_ascii=False,indent=2));return 0 if ok else 2
