...
def main():
 c=[];auth=load(PH/'config/final_authority_map.json',{});c.append(ck('one canonical owner per major authority',all(x.get('canonical_count')==1 for x in auth.get('authorities',[])) and len({x['domain'] for x in auth.get('authorities',[])})==10));const=load(PH/'config/final_scientific_constitution.json',{});c.append(ck('scientific constitution consolidated',len(const.get('invariants',[]))>=24));fr=verify_manifest(REPO);c.append(ck('science freeze valid',fr.get('classes',{}).get('SCIENTIFIC_IMMUTABLE',{}).get('status')=='PASS'));c.append(ck('runtime freeze valid',fr.get('classes',{}).get('RUNTIME_IMMUTABLE',{}).get('status')=='PASS'));c.append(ck('report freeze valid',fr.get('classes',{}).get('REPORT_CONTRACT_IMMUTABLE',{}).get('status')=='PASS'));c.append(ck('certification governance freeze valid',fr.get('classes',{}).get('CERTIFICATION_IMMUTABLE',{}).get('status')=='PASS'));pol=load(PH/'config/final_promotion_gate_registry.json',{});decl=tuple(x['gate_id'] for x in pol.get('gates',[]));c.append(ck('declared gates equal machine gates',decl==GATE_IDS));c.append(ck('automatic promotion forbidden',pol.get('automatic_promotion_forbidden') is True));c.append(ck('P09 forward gate is mandatory','P09_FORWARD_EVIDENCE_PROVISIONAL' in decl));c.append(ck('explicit approval is mandatory','EXPLICIT_OPERATOR_APPROVAL' in decl));rp=load(NEXT/'AD_V3_PHASE_10_UNIFIED_RUNTIME_ONE_RUN/config/route_policy.json',{});c.append(ck('production routing source remains P10',rp.get('routing_authority')=='AD-V3-P10'));rc=load(NEXT/'AD_V3_PHASE_11_FINAL_INSTITUTIONAL_GOLD_CONTROL_ROOM/config/report_contract.json',{});c.append(ck('canonical V3 HTML remains P11',rc.get('canonical_v3_html_authority')=='AD-V3-P11'));c.append(ck('trade execution authority NONE',auth.get('trade_execution_authority')=='NONE'))
 # Obvious secret and absolute-path scan in canonical V3 source/config surfaces.
 files=[]
 for d in [NEXT/'AD_V3_PHASE_03_CAUSAL_GOLD_BRAIN_DECISION_ENGINE',NEXT/'AD_V3_PHASE_06_GOVERNED_SEMANTIC_INTELLIGENCE',NEXT/'AD_V3_PHASE_07_LIVE_INTRADAY_GOLD_DATA_KERNEL',NEXT/'AD_V3_PHASE_08_DECISION_SCIENCE_CALIBRATION',NEXT/'AD_V3_PHASE_09_TRUE_FORWARD_VALIDATION_2_0',NEXT/'AD_V3_PHASE_10_UNIFIED_RUNTIME_ONE_RUN',NEXT/'AD_V3_PHASE_11_FINAL_INSTITUTIONAL_GOLD_CONTROL_ROOM',PH]:
 ...
 ok=all(x['status']=='PASS' for x in c);out={'phase':'AD-V3-P12','version':'3.12.0-final-certification-freeze','status':'PASS' if ok else 'FAIL_CLOSED','check_count':len(c),'checks':c};print(json.dumps(out,ensure_ascii=False,indent=2));return 0 if ok else 2
