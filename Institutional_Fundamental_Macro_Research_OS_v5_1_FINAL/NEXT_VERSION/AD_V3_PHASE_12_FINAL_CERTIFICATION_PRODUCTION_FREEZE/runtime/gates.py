...
def _acc(phase):
 p=list(NEXT.glob('AD_V3_PHASE_'+phase+'_*'))[0]/'tools'/('run_phase'+phase+'_acceptance.py')
 cp=subprocess.run([sys.executable,str(p)],capture_output=True,text=True,encoding='utf-8',errors='replace',timeout=300)
...
def collect(approve=False,deep=False,repo=REPO):
 fr=verify_manifest(repo);p09=real_p09();fs=(p09.get('statistics') or {}).get('forward_evidence_state','UNAVAILABLE');p11=load(NEXT/'AD_V3_PHASE_11_FINAL_INSTITUTIONAL_GOLD_CONTROL_ROOM/PHASE_11_HANDOFF.json',{}) or {};cert=_static_cert();gold=load(PHASE/'certification/GOLDEN_CERTIFICATION_RECEIPT.json',{}) or {};rb=load(PHASE/'certification/ROLLBACK_CERTIFICATION_RECEIPT.json',{}) or {};rt=load(PHASE/'certification/ROUTING_CERTIFICATION_RECEIPT.json',{}) or {}
 ...
 'P09_FORWARD_EVIDENCE_PROVISIONAL':fs in ('PROVISIONAL','MATURE'),'GOLDEN_CERTIFICATION_PASS':gold.get('status')=='PASS','ROLLBACK_CERTIFIED':rb.get('status')=='PASS','PRODUCTION_ROUTING_CERTIFIED':rt.get('status')=='PASS','V2_ROLLBACK_AVAILABLE':rb.get('v2_p11')=='PASS' and rb.get('v2_p13')=='PASS','P11_VISUAL_ACCEPTANCE_PASS':str(p11.get('visual_acceptance','')).startswith('PASS'),'P11_SECURITY_ACCEPTANCE_PASS':p11.get('security_acceptance')=='PASS','EXPLICIT_OPERATOR_APPROVAL':approve is True}
 # aliases from phase loop to final gate ids
 for i in range(1,12):g[f'P{i:02d}_PASS']=g.get(f'P{i:02d}_PASS',False)
 # registry uses P09_IMPLEMENTATION not P09_PASS but can retain both internal.
 result={k:g.get(k,False) for k in GATE_IDS};return result,{'P09':p09,'freeze':fr,'forward_state':fs}
def enforce(gates):
...
def certification_state(repo=REPO):
 ...
 p04=NEXT/'AD_V3_PHASE_04_CONTROL_ROOM_TRUE_FORWARD_COMMISSIONING';state=(load_state(p04) or {}).get('state','SHADOW_COMMISSIONING');cert=_static_cert();fr=verify_manifest(repo);p09=real_p09();fs=(p09.get('statistics') or {}).get('forward_evidence_state','UNAVAILABLE')
 ...
 if fs not in ('PROVISIONAL','MATURE'):return 'CERTIFIED_PENDING_FORWARD_EVIDENCE'
