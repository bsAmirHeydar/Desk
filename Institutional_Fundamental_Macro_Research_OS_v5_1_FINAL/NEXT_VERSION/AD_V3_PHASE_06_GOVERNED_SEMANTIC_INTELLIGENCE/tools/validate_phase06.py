...
def main():
    ...
    pipeline=(p04/'runtime/pipeline.py').read_text(encoding='utf-8'); checks.append(check('P04 commissioning wired to P06 runtime','AD_V3_PHASE_06_GOVERNED_SEMANTIC_INTELLIGENCE.runtime.semantic_runtime' in pipeline and 'run_semantics(repo, packet' in pipeline and 'semantic_validation_receipt.json' in pipeline))
    launcher=(REPO/'AlphaDesk.ps1').read_text(encoding='utf-8-sig'); checks.append(check('launcher exposes v3 semantic status','v3-semantic-status' in launcher))
