...
def execute(phase_root, p02_data_root, coverage_path=None, adjudication_path=None, horizon='SESSION_1_6H'):
    ...
    receipt={'record_type':'AD_V3_P03_CAUSAL_BRAIN_RECEIPT','receipt_id':stable_id('P03BRAIN',{'coverage':coverage.get('receipt_id'),'horizon':horizon,'decision':decision}),'phase':'AD-V3-P03','subject':'XAUUSD','generated_at_utc':iso(),'horizon':horizon,'p02_coverage_receipt_id':coverage.get('receipt_id'),'handoff_integrity':handoff_integrity,'analysis_admission':'DEGRADED' if coverage.get('analysis_admission')=='DEGRADED' or unresolved else 'PASS','reasoning_ledger':{'record_type':'AD_V3_P03_REASONING_LEDGER','fact_count':192,'rows':rows,'unresolved_fact_ids':unresolved,'zero_silent_omission':True},'pressure_planes':{'causal_fundamental':causal,'realized_transaction':transaction,'mechanical_forced':mechanical,'structural_carry':structural},'price_transmission':transmission,'model_quality':quality,'lifecycle':{'consumption_vector':consumption,'persistence_stack':persistence,'remaining_pressure':remaining},'hypotheses':hypotheses,'semantic_adjudication_request':adjudication_request,'shadow_decision':decision,'production_direction_authority_granted':False,'production_trade_permission_authority_granted':False,'hard_note':'P03 is SHADOW_ONLY. Direction/action are research candidates, not production permission.'}
    return receipt
