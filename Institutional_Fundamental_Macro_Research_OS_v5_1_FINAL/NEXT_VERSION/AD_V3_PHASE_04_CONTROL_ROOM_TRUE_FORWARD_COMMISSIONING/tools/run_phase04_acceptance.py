...
def main():
    ...
    checks.append(ck('commissioning CLI stops cleanly without raw traceback',(('AD_V3_PHASE_10_UNIFIED_RUNTIME_ONE_RUN' in cli_src and 'subprocess.run' in cli_src) if (N/'AD_V3_PHASE_10_UNIFIED_RUNTIME_ONE_RUN').exists() else ('except RuntimeError as e:' in cli_src and 'COMMISSIONING - STOPPED' in cli_src))))
    checks.append(ck('V3 CLI forces UTF-8 operator stdout and stderr',(('PYTHONIOENCODING' in (REPO/'AlphaDesk.ps1').read_text(encoding='utf-8-sig') and 'PYTHONUTF8' in (REPO/'AlphaDesk.ps1').read_text(encoding='utf-8-sig')) if (N/'AD_V3_PHASE_10_UNIFIED_RUNTIME_ONE_RUN').exists() else ("def _configure_utf8_stdio" in cli_src and "reconfigure(encoding='utf-8', errors='replace')" in cli_src))))
