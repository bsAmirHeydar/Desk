...
def render(model,help_registry):
    e=model['executive_state']; q=model['model_quality']; perm=model['permission']; life=model.get('lifecycle') or {}; tf=model.get('true_forward') or {}; dk=model.get('data_kernel') or {}; dc=model.get('decision_calibration') or {}
...
<section class="section"><div class="head"><h2>Decision Calibration</h2><span class="muted">P03 causal state → P08 calibrated authority</span></div><div class="grid4"><div class="metric"><span>Strength</span><b>{esc(dc.get('pressure_strength'))}</b></div><div class="metric"><span>Dominance</span><b>{esc(dc.get('dominance_state'))}</b><small>{esc(dc.get('dominant_root'))}</small></div><div class="metric"><span>Breadth / Fragility</span><b>{esc(str(dc.get('breadth'))+' / '+str(dc.get('fragility')))}</b></div><div class="metric"><span>Edge</span><b>{esc(dc.get('edge_state'))}</b><small>{esc(dc.get('contradiction'))}</small></div></div></section>
...
def brief(model):
    e=model['executive_state']; q=model['model_quality']; p=model['permission']; tf=model['true_forward']; comp=model.get('comparison') or {}; dk=model.get('data_kernel') or {}; dc=model.get('decision_calibration') or {}
    reason=e['summary']
    uncertainty=[]
    ...
    return '\n'.join([
        ...
        f"Decision Calibration: {dc.get('pressure_strength')} · {dc.get('dominance_state')} · root {dc.get('dominant_root')} · breadth {dc.get('breadth')} · fragility {dc.get('fragility')} · edge {dc.get('edge_state')}",
        compared,
        '',
