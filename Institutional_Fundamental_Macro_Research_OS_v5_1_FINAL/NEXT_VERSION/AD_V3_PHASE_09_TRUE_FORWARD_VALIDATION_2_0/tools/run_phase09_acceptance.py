...
def main():
    ...
    b=json.loads((P/'baseline/PRE_P09_AUTHORITY_HASHES.json').read_text());h=__import__('hashlib');p03_same=(b['hashes']['p03_reasoning_registry']==h.sha256((N/'AD_V3_PHASE_03_CAUSAL_GOLD_BRAIN_DECISION_ENGINE/config/fact_reasoning_registry.json').read_bytes()).hexdigest());p08_now=h.sha256((N/'AD_V3_PHASE_08_DECISION_SCIENCE_CALIBRATION/config/decision_calibration_policy.json').read_bytes()).hexdigest();p08_same=(b['hashes']['p08_decision_policy']==p08_now)
    arch=json.loads((N/'AD_V3_PHASE_05_INTEGRITY_ARCHITECTURE_CONSOLIDATION/config/architecture_surface_registry.json').read_text());rel='Institutional_Fundamental_Macro_Research_OS_v5_1_FINAL/NEXT_VERSION/AD_V3_PHASE_08_DECISION_SCIENCE_CALIBRATION/config/decision_calibration_policy.json';own=next((x for x in arch.get('surfaces',[]) if x.get('path')==rel),{})
    r02_versioned=(own.get('class')=='SCIENTIFIC_VERSIONED' and str(own.get('owner','')).startswith('AD-V3.1-R02'))
    checks.append(ck('substantive Gold drift zero and P08 change is explicit R02 scientific amendment',p03_same and (p08_same or r02_versioned),{'p03_same':p03_same,'p08_same':p08_same,'p08_owner':own.get('owner'),'p08_current_sha256':p08_now}))
