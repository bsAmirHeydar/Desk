...
def compatible(pred,o):
    ...
    ps=pa.get('scientific_series_id');os=o.get('scientific_series_id')
    # R04 scientific equivalence: provider identity may differ for the SAME canonical market series.
    # If both sides carry a series identity, that identity governs outcome compatibility.
    if ps and os:
        if ps!=os:return False
        if pa.get('instrument_key') and o.get('instrument_key') and o.get('instrument_key')!=pa.get('instrument_key'):return False
        return True
    # Legacy records remain immutable and keep their strict same-source compatibility.
