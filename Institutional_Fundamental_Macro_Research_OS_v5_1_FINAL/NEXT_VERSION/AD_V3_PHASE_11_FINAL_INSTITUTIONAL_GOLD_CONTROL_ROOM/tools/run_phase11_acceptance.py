...
def main():
 ...
 with tempfile.TemporaryDirectory() as td:
  ...
  required=['now','changed','drivers','pressure','transmission','watch','invalidation','events','data','forward','evidence','audit'];checks.append(ck('all human-first sections present',all(soup.find(id=x) for x in required),required))
 ...
 pre_dag=load(PH/'baseline/PRE_P11_P10_STAGE_DAG.json');cur_dag=load(NEXT/'AD_V3_PHASE_10_UNIFIED_RUNTIME_ONE_RUN/config/stage_dag.json');pre_top=[(x['stage_id'],tuple(x.get('depends_on') or []),x.get('mandatory')) for x in pre_dag['stages']];cur_top=[(x['stage_id'],tuple(x.get('depends_on') or []),x.get('mandatory')) for x in cur_dag['stages']];auth={x['stage_id']:x.get('authority') for x in cur_dag['stages']};checks.append(ck('P10 stage DAG topology unchanged and report authority moved to P11',pre_top==cur_top and auth.get('CONTROL_MODEL')=='AD-V3-P11' and auth.get('REPORT')=='AD-V3-P11',auth))
 ...
 ok=all(x['status']=='PASS' for x in checks);out={'phase':'AD-V3-P11','version':'3.11.0-final-control-room','acceptance_status':'PASS' if ok else 'FAIL_CLOSED','check_count':len(checks),'checks':checks,'canonical_renderer':'AD-V3-P11','visual_acceptance':next((x['detail'].get('status') for x in checks if x['name'].startswith('browser visual') and isinstance(x.get('detail'),dict)),'UNKNOWN'),'v3_state':'SHADOW_COMMISSIONING','production_promotion_performed':False,'trade_execution_authority':'NONE'};print(json.dumps(out,ensure_ascii=False,indent=2,default=str));return 0 if ok else 2
