...
class RetrievalRuntime:
    ...
    def _requirements(self,run_id):
        ...
        for f in cov['fact_families']:
            fam=self.fmap.get(f['family_id'],{});req.append({'requirement_id':'REQ_'+f['family_id'],'fact_family':f['family_id'],'applicability':f['applicability'],'materiality':f['materiality'],'reason':f['reason'],'source_ids':list(fam.get('direct_or_primary_source_ids',[])),'proxy_source_ids':list(fam.get('proxy_or_context_source_ids',[])),'licensed_source_ids':list(fam.get('licensed_or_private_source_ids',[]))})
        return req
    ...
    def _external(self,b,request):
        ...
        if not cmd:return []
        d=self.rt.data_root/'tmp'/'r3_retrieval';d.mkdir(parents=True,exist_ok=True);rid=uid('RET');req=d/(rid+'.request.json');rsp=d/(rid+'.response.json');dump_json(req,request);parts=shlex.split(cmd,posix=(os.name!='nt'))+[str(req),str(rsp)]
    ...
    def _rows_for_source(self,sid,r,run_mode,cutoff,host_retriever=None):
        b=self._binding(sid);a=b.get('adapter','UNBOUND');request={'schema_version':'1.0.0','operation':'EVIDENCE_RETRIEVAL','analysis_cutoff_utc':cutoff,'capture_mode':('LIVE_PRE_RUN' if run_mode in ('LIVE','SHADOW_LIVE') and cutoff is None else 'FROZEN_CUTOFF'),'run_mode':run_mode,'requirements':[r],'source_contracts':[self.smap.get(sid,{'source_id':sid})],'strict_point_in_time':True}
        try:
    ...
    def capture(self,run_id,host_retriever=None,live_intake=None):
        ...
        for s in allrows:
            ...
            if role!='PROXY':candidates.append(cand)
