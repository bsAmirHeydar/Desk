...
def expected_outcome_contract(research_class,horizon):
    ...
    if rc in ('DIRECTIONAL_FORECAST','PERSISTENCE_REVERSAL','CROSS_SECTIONAL_RELATIVE_VALUE'): typ='DIRECTIONAL_OR_RELATIVE_OUTCOME'
...
def _apl_snapshot(rt,run_id,telemetry,bundle):
    payloads={}
    for name in sorted((bundle or {}).get('artifacts',{})):
        x=_artifact(rt,run_id,name,None)
        if isinstance(x,(dict,list)):payloads[name]=x
    return {
        'telemetry_snapshot':telemetry if isinstance(telemetry,dict) else {},
        'active_lenses':list((telemetry or {}).get('active_lenses') or []),
        'material_findings':list((telemetry or {}).get('material_findings') or []),
        'would_request':list((telemetry or {}).get('would_request') or []),
...
def seal_run(vault_root,run_id,truth_state='SHADOW_LIVE'):
    ...
    plan=_artifact(rt,run_id,'method_plan',{}) or {};mpre=_artifact(rt,run_id,'method_predecision_validation_receipt',{}) or {}
    telemetry=_artifact(rt,run_id,'apl_a_forward_telemetry',{}) or {}; bundle=_artifact(rt,run_id,'apl_a_shadow_bundle',{}) or {}
    ri=_artifact(rt,run_id,'research_intent',{}) or {};fp=_artifact(rt,run_id,'final_permission',{}) or {};uireq=_artifact(rt,run_id,'unified_request',{}) or {};cintent=_artifact(rt,run_id,'compiled_intent',{}) or {};cresult=_artifact(rt,run_id,'canonical_scientific_result',{}) or {};rcap=_artifact(rt,run_id,'run_capsule',{}) or {};rqual=_artifact(rt,run_id,'run_quality_receipt',{}) or {}
    commitment_id='TF1_'+run_id
    method_snapshot={
        'version':plan.get('method_version','M1.0.1'),'protocol_id':plan.get('protocol_id'),'rigor_tier':plan.get('rigor_tier'),
        'predecision_status':mpre.get('status'),'findings_snapshot':list(mpre.get('findings') or []),
        'hard_failure_count':mpre.get('hard_failure_count',0),'source_dependency_graph':mpre.get('source_dependency_graph') or {},
    }
    apl_snapshot={'version':'APL-A 1.0.0','mode':'SHADOW_ONLY','artifact_present':bool(telemetry or bundle),**_apl_snapshot(rt,run_id,telemetry,bundle)}
    base={'schema_version':'1.0.0','true_forward_version':TF_VERSION,'commitment_id':commitment_id,'run_id':run_id,'truth_state':truth_state,'state':'SEALED_PRE_OUTCOME','reviewability':'FULL_M1_APL_SNAPSHOT','subject':m.get('subject'),'research_class':plan.get('research_class'),'horizon':plan.get('active_horizon') or ri.get('active_horizon'),'analysis_cutoff_utc':m.get('analysis_cutoff_utc'),'evidence_cutoff_utc':m.get('analysis_cutoff_utc'),'sealed_at_utc':sealed_at,'decision_seal_hash':m.get('decision_seal_hash'),'method':method_snapshot,'apl_a':apl_snapshot,'science':{'fundamental_direction':ri.get('fundamental_direction'),'edge_state':ri.get('edge_state'),'permission':fp.get('permission'),'artifact_refs':_artifact_refs(rt,run_id),'unresolved_contradictions':ri.get('unresolved_contradictions') or []},'uncertainty':ri.get('uncertainty_profile') or ri.get('uncertainty'),'expected_outcome_contract':expected_outcome_contract(plan.get('research_class'),plan.get('active_horizon')),'authority':{'direction_mutated':False,'permission_mutated':False,'broker_write':'NONE','apl_a':'SHADOW_ONLY','m1_new_direction_authority':'NONE'},'interface':{'version':'UI1.0.0' if uireq or cintent or cresult else None,'request_id':cintent.get('request_id') or uireq.get('request_id'),'original_request':uireq if uireq else None,'compiled_intent':cintent if cintent else None,'canonical_result_hash':cresult.get('canonical_result_hash') if isinstance(cresult,dict) else None,'run_contract_version':'RUN2.0.0' if rcap else None,'run_capsule_hash':rcap.get('capsule_hash') if isinstance(rcap,dict) else None,'run_quality_status':rqual.get('status') if isinstance(rqual,dict) else None},'source_surface_hash':current_certification_surface(v)}
    ...
    return {'status':'PASS','commitment_id':commitment_id,'truth_state':truth_state,'seal':base['seal'],'reviewability':base['reviewability'],'path':str(p)}
...
def verify_commitment(vault_root,commitment_id):
    ...
    return {'status':'PASS' if not errors else 'FAIL','commitment_id':commitment_id,'truth_state':obj.get('truth_state'),'true_forward_version':obj.get('true_forward_version'),'seal':obj.get('seal'),'path':str(p),'errors':errors}
...
def list_records(vault_root):
    ...
    for p in sorted(ds['commitments'].glob('TF1_*.json')):
        try:
            c=load_json(p);vr=verify_commitment(v,c['commitment_id']);o=outcomes.get(c['commitment_id'])
            rows.append({'commitment_id':c['commitment_id'],'run_id':c['run_id'],'subject':c['subject'],'truth_state':c['truth_state'],'true_forward_version':c.get('true_forward_version','TF1.0.0'),'reviewability':c.get('reviewability') or ('FULL_M1_APL_SNAPSHOT' if c.get('method',{}).get('findings_snapshot') is not None and c.get('apl_a',{}).get('artifact_payloads') is not None else 'LIMITED_REVIEWABILITY'),'state':'OUTCOME_LINKED' if o else 'WAITING_FOR_OUTCOME','sealed_at_utc':c['sealed_at_utc'],'seal_valid':vr['status']=='PASS','outcome_observed_at_utc':o.get('observed_at_utc') if o else None})
        except Exception as e:rows.append({'path':str(p),'state':'INVALID','error':str(e)})
    mature=sum(1 for r in rows if r.get('state')=='OUTCOME_LINKED' and r.get('truth_state')=='TRUE_FORWARD');tf=sum(1 for r in rows if r.get('truth_state')=='TRUE_FORWARD')
    maturity='INSUFFICIENT' if mature==0 else ('EARLY' if mature<10 else 'DEVELOPING')
...
def _test_record(root,truth='TRUE_FORWARD',version=TF_VERSION):
    base={'schema_version':'1.0.0','true_forward_version':version,'commitment_id':'TF1_TEST','run_id':'TEST','truth_state':truth,'state':'SEALED_PRE_OUTCOME','reviewability':'FULL_M1_APL_SNAPSHOT' if version=='TF1.0.1' else 'LIMITED_REVIEWABILITY','subject':'XAUUSD','research_class':'DIRECTIONAL_FORECAST','horizon':'DAILY','analysis_cutoff_utc':'2026-08-10T08:00:00Z','evidence_cutoff_utc':'2026-08-10T08:00:00Z','sealed_at_utc':'2026-08-10T08:01:00Z','decision_seal_hash':'sha256:'+'1'*64,'method':{'version':'M1.0.1','protocol_id':'P','findings_snapshot':[]},'apl_a':{'version':'APL-A 1.0.0','mode':'SHADOW_ONLY','active_lenses':[],'artifact_payloads':{}},'science':{},'uncertainty':None,'expected_outcome_contract':expected_outcome_contract('DIRECTIONAL_FORECAST','DAILY'),'authority':{'direction_mutated':False,'permission_mutated':False,'broker_write':'NONE','apl_a':'SHADOW_ONLY','m1_new_direction_authority':'NONE'},'source_surface_hash':None}
...
def selftest(vault_root):
    ...
    v=Path(vault_root).resolve();pol=load_json(v/'RUNTIME'/'Production Commissioning'/'config'/'true_forward_policy.json')
    ck('policy_authority_none',pol['broker_write'] is False and pol['direction_authority_added']=='NONE' and pol['apl_a_authority']=='SHADOW_ONLY')
    ck('source_mutable_separation',pol['mutable_forward_records_in_r4_source_fingerprint'] is False)
    ck('snapshot_policy_enabled',pol.get('snapshot_method_findings') is True and pol.get('snapshot_apl_output_payloads') is True)
    ...
    try:
        ...
        ck('new_record_full_reviewability',load_json(cp).get('reviewability')=='FULL_M1_APL_SNAPSHOT')
        b=cp.read_bytes()
