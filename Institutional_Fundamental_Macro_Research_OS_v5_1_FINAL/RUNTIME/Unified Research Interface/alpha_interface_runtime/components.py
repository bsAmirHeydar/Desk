...
def method_and_apl(model):
    mh=model['method_health'];p=model['perspective'];rq=model.get('run_quality') or {};apl='<p>در حال حاضر نقطه شکنندگی مهمی در تحلیل شناسایی نشده است.</p>' if p.get('quiet') else '<ul>'+li(p.get('material_findings_fa'))+'</ul>'
    rq_html=f'''<details><summary><span>سلامت اجرای تحلیل</span><small>کیفیت فرایند ≠ اطمینان بازار</small></summary><p>{e(rq.get('simple_summary_fa'))}</p><dl><div><dt>وضعیت</dt><dd>{e(rq.get('status') or 'نامشخص')}</dd></div><div><dt>Run Capsule</dt><dd>{e((rq.get('capsule') or {}).get('capsule_hash') or 'هنوز ثبت نشده')}</dd></div></dl><details class="nested"><summary>چک‌لیست کامل RUN2</summary><pre>{e(json.dumps(rq.get('receipt') or {},ensure_ascii=False,indent=2))}</pre></details></details>'''
    return f'''<section class="shell health">{rq_html}<details><summary><span>کیفیت و سلامت تحلیل</span><small>این بخش جهت بازار را تعیین نمی‌کند.</small></summary><p>{e(mh.get('simple_summary_fa'))}</p><dl><div><dt>کلاس تحقیق</dt><dd>{e(mh.get('research_class'))}</dd></div><div><dt>سطح سخت‌گیری</dt><dd>{e(mh.get('rigor_tier'))}</dd></div><div><dt>تعداد خطای سخت</dt><dd>{e(mh.get('hard_failure_count'))}</dd></div></dl><details class="nested"><summary>جزئیات روش و شواهد</summary><pre>{e(json.dumps(mh,ensure_ascii=False,indent=2))}</pre></details></details>
