...
def run(vault_root):
    ...
    ck('ux3_architecture','market-strip' in h and 'workspace' in h and 'lens-index' in h and 'deep-drawer' not in h and 'id="drawer"' in h)
    ck('command_center_request_aware','نرخ واقعی یکی از توضیح‌های اصلی است' in h and 'تصویر فعلی بازار' in h)
    ck('lens_index_eight_sections',all(x in h for x in ['وضعیت بنیادی','روایت، ذهن بازار و میزان مصرف خبر','موقعیت معامله‌گران و شلوغی بازار','جریان واقعی خرید و فروش','نقدینگی و تأمین مالی','ساختار، ظرفیت و نوسان بازار']))
    ck('single_reusable_drawer','id="drawer"' in h and h.count('role="dialog"')==1 and '<dialog' not in h and 'technical_detail' not in h)
    ck('rtl','lang="fa" dir="rtl"' in h)
    ck('accessibility','aria-modal="true"' in h and 'aria-label="بستن"' in h and 'prefers-reduced-motion' in h and 'focus-visible' in h)
    ck('perspective_separate','این بخش جهت بازار را تعیین نمی‌کند' in h and model['layer1']['direction']=='BEARISH')
    ck('no_fake_probability','67%' not in h)
    ck('no_visible_engineering_metadata',all(x not in h for x in ['RUN2','R2 Prompt','R3','technical_detail','artifact_hash','decision_seal_hash','vault_commit']))
    ck('scenario_navigator','scenario-tabs' in h and 'scenario-canvas' in h)
    ck('know_interpret_unknown',all(x in h for x in ['می‌دانیم','برداشت فعلی','هنوز نمی‌دانیم']))
    ck('interface_manifest',json.loads((v/'RUNTIME'/'Unified Research Interface'/'UNIFIED_INTERFACE_MANIFEST.json').read_text(encoding='utf-8'))['version']=='UX3.0.0')
