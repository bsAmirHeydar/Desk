...
def build(rt,run_id,canonical,compiled):
    ...
    direction=canonical['decision'].get('final_direction') or canonical['decision'].get('fundamental_direction');permission=canonical['decision'].get('permission');remaining=canonical['science'].get('remaining_asymmetry')
    ...
    changes=_vals(dt,'changes','changed_components','transition_reasons','new_information',limit=6)
    layer2_pressure={'force':ri.get('urgency'),'force_fa':to_fa(ri.get('urgency')),'consumption':cons.get('lifecycle_state'),'consumption_fa':to_fa(cons.get('lifecycle_state')),'remaining_pressure':cons.get('remaining_asymmetry') or ri.get('remaining_asymmetry'),'remaining_pressure_fa':to_fa(cons.get('remaining_asymmetry') or ri.get('remaining_asymmetry')),'remaining_pressure_reasons':[simplify_text(x) for x in cons.get('remaining_asymmetry_reasons',[]) if simplify_text(x)],'persistence':ri.get('validity_action'),'persistence_fa':to_fa(ri.get('validity_action')),'reversal':ri.get('reversal_hazard'),'reversal_fa':to_fa(ri.get('reversal_hazard')),'invalidation_triggers':invalidation}
    report={'schema_version':'1.0.0','report_model_version':'2.0.0','interface_version':'UI2.0.0',
      ...
      'layer1':{'focus':focus,'direction':direction,'direction_fa':to_fa(direction),'permission':permission,'permission_fa':to_fa(permission),'force':ri.get('urgency'),'force_fa':to_fa(ri.get('urgency')),'consumption':cons.get('lifecycle_state'),'consumption_fa':to_fa(cons.get('lifecycle_state')),'remaining_pressure':remaining,'remaining_pressure_fa':to_fa(remaining),'persistence':first(ri,'validity_action',default='UNDETERMINED'),'persistence_fa':to_fa(first(ri,'validity_action',default='UNDETERMINED')),'reversal_risk':canonical['science'].get('reversal_hazard'),'reversal_risk_fa':to_fa(canonical['science'].get('reversal_hazard')),'dominant_driver':simplify_text(primary_driver) or None,'what_matters_now_fa':('مهم‌ترین عامل فعلی: '+simplify_text(primary_driver)+'.') if simplify_text(primary_driver) else 'عامل غالب هنوز با اطمینان کافی مشخص نشده است.','what_would_change_this_fa':('این تحلیل باید بازبینی شود اگر '+invalidation[0]) if invalidation else 'عامل مشخص و قابل اتکایی برای ابطال تحلیل در این snapshot ثبت نشده است.','next_review':next_review,'next_review_fa':to_fa(next_review)},
      'decision_strip':{'direction':direction,'fundamental_direction':canonical['decision'].get('fundamental_direction'),'permission':permission,'edge_state':canonical['decision'].get('edge_state'),'pressure_strength':ri.get('urgency') or 'UNDETERMINED','consumption':cons.get('lifecycle_state') or 'UNDETERMINED','remaining_pressure':remaining,'persistence':first(ri,'validity_action',default='UNDETERMINED'),'reversal_risk':canonical['science'].get('reversal_hazard'),'next_review':next_review},
      'layer2':{'cards':cards,'drivers':{'primary':primary_driver,'primary_fa':simplify_text(primary_driver),'secondary':secondary,'secondary_fa':simplify_text(secondary),'opposing':opposing,'opposing_fa':simplify_text(opposing),'transition':dt},'causal_chain':_causal_chain(causal),'pressure':layer2_pressure,'changes':{'items':changes,'has_comparison':bool(changes),'empty_message_fa':'مقایسه معتبر با اجرای قبلی در این snapshot ثبت نشده است.'},'scenarios':scenarios},
