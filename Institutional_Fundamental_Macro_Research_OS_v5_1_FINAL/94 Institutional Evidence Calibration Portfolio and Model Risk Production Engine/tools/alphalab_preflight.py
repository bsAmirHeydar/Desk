...
if m.get('current_stack') not in {'V16.1.0','V17.0.0','V18.0.0','V19.0.0','V20.0.0','V21.0.0','V21.1.0','V21.2.0','V21.3.0'}: errors.append('current_stack is not a supported V16.1/V17.0/V18.0/V19.0/V20.0/V21.x stack')
if m.get('current_stack') in {'V17.0.0','V18.0.0','V19.0.0','V20.0.0','V21.0.0','V21.1.0','V21.2.0','V21.3.0'} and 'fact_constitution' not in (m.get('canonical_authorities') or {}): errors.append('V17+ fact_constitution authority missing')
if m.get('current_stack') in {'V18.0.0','V19.0.0','V20.0.0','V21.0.0','V21.1.0','V21.2.0','V21.3.0'}:
    ...
    if m.get('current_stack') in {'V19.0.0','V20.0.0','V21.0.0','V21.1.0','V21.2.0','V21.3.0'} and d2.get('direct_permission_effect')!='NONE': errors.append('V19 D2 direct permission boundary violated')
if m.get('current_stack') in {'V21.0.0','V21.1.0','V21.2.0','V21.3.0'}:
    ca=m.get('canonical_authorities') or {}
    ...
    if ch.get('authority_mode')!='ENFORCED_COGNITIVE_HARDENING': errors.append('V21 cognitive hardening not enforced')
    if ch.get('direction_flip_allowed') is not False: errors.append('V21 cognitive direction flip guard drift')
    if ch.get('positive_permission_creation') is not False: errors.append('V21 cognitive positive permission guard drift')
