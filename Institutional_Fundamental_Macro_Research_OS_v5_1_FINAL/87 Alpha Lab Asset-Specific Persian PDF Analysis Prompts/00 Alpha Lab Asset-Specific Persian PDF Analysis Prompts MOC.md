---
title: "Alpha Lab Asset-Specific Persian PDF Analysis Prompts"
type: moc
status: canonical
version: 13.0.0
created: 2026-07-30
updated: 2026-08-06
language: en
tags: [alpha-lab, prompts, pdf, persian, institutional-analysis]
---
# Alpha Lab Asset-Specific Persian PDF Analysis Prompts

This module contains four standalone English production prompts. Each prompt requires the attached Institutional Fundamental Macro Research OS Vault, performs a fundamental-only institutional analysis, and creates an actual Persian right-to-left PDF report under the **Alpha Lab** brand.

## Primary prompts

- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/04 Nasdaq 100 Alpha Lab Persian PDF Prompt]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/05 S&P 500 Alpha Lab Persian PDF Prompt]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/06 Gold Alpha Lab Persian PDF Prompt]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/07 EURUSD Alpha Lab Persian PDF Prompt]]

## Shared operating standards

- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/01 Alpha Lab Persian PDF Design System]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/02 Shared Vault Retrieval Evidence and Analysis Contract]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/03 PDF Production Validation and Delivery Contract]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/08 Fast Launcher Messages]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/09 Input Reference and Worked Examples]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/10 Report Naming Archiving and Reuse Standard]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/11 Daily Fundamental Context Engine]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/12 Daily Persian PDF Report Contract]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/13 Daily Fast Launcher Messages]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/14 Hourly Fundamental State Engine]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/15 Hourly Direction Intensity Consumption and Remaining Pressure Standard]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/16 Hourly Persian PDF Report Contract]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/17 Hourly Fast Launcher Messages]]
- [[87 Alpha Lab Asset-Specific Persian PDF Analysis Prompts/18 Hourly Fundamental State Output Schema and Worked Interpretation]]

## Operating principle

The prompt text is English to maximize instruction precision. The final report is Persian, right-to-left, visually consistent and intentionally simple in its main conclusions. Institutional detail, citations, the Vault Reading Ledger and methodological limitations belong in appendices rather than cluttering the executive pages.

## Mandatory boundary

These prompts are fundamental-only. They may use price and market data to measure valuation, market-implied expectations, volatility, liquidity, basis, positioning proxies and realized response. They may not use chart patterns, indicators or non-fundamental entry systems.

## Daily operating mode

The four asset prompts support daily pre-market, live-session, post-event and end-of-day profiles. Daily reports prioritize the current trading day and use the structural canon only as an inherited constraint.


## Hourly operating mode

The four asset prompts support fixed hourly snapshots plus event-driven updates. Hourly outputs separate direction, intensity, confidence, consumption, remaining pressure, persistence, reversal risk and no-material-update states.
## Hybrid historical and intraday state engine

The canonical dense reconstruction architecture is [[88 Hybrid Daily Session Event Fundamental State Engine/00 Hybrid Daily Session Event Fundamental State Engine MOC]]. It governs mandatory daily baselines, session handoffs, event micro-windows, state decay, no-change reassessments and density validation.

---

## V11 analysis engine

The asset-specific Persian reporting layer may continue to use its existing output contracts. Its analytical input should now be produced by:

- [[89 Fundamental Force Consumption Persistence and Asymmetry Calibration Engine/38 Alpha Lab V11 Full-Spectrum Fundamental State Analysis Prompt]]
- [[89 Fundamental Force Consumption Persistence and Asymmetry Calibration Engine/25 Nasdaq 100 Force Consumption and Persistence Book]]
- [[89 Fundamental Force Consumption Persistence and Asymmetry Calibration Engine/26 S&P 500 Force Consumption and Persistence Book]]
- [[89 Fundamental Force Consumption Persistence and Asymmetry Calibration Engine/27 Gold Force Consumption and Persistence Book]]
- [[89 Fundamental Force Consumption Persistence and Asymmetry Calibration Engine/28 EURUSD Force Consumption and Persistence Book]]

The Persian PDF format is preserved; the state methodology is upgraded.

## V12 narrative-intelligence integration

Asset-specific Persian reports should use [[90 Market Narrative Intelligence Engine/41 Alpha Lab V12 Daily Fundamental and Narrative Analysis Prompt]] and the relevant Module 90 asset book before rendering the executive PDF.



## V13 universal multi-asset coverage

Module 91 adds universal resolution, FX/commodity/index adapters, watchlists, ranking and relative value. Start at [[91 Global Multi-Asset Coverage and Instrument Intelligence Engine/00 Global Multi-Asset Coverage and Instrument Intelligence Engine MOC]]. V11 and V12 authorities remain intact.
