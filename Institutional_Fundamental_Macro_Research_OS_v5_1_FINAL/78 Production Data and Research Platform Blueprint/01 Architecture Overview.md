---
title: "01 Architecture Overview"
type: platform-specification
status: evergreen
version: 6.0.0
created: 2026-07-29
updated: 2026-07-29
language: en
tags: [data-platform, research-engineering]
---
# 01 Architecture Overview

## Purpose

A layered platform separates source ingestion, bitemporal storage, transformations, feature production, models, claims, decisions and monitoring.

## Components

Source zone; canonical warehouse; feature store; model registry; research packages; decision ledger; live monitoring.

## Interfaces

The service publishes versioned schemas, lineage events, quality states, ownership metadata and machine-readable contracts. Upstream and downstream dependencies are explicit.

## Acceptance gate

Lineage traversal works end to end; every production output can be reconstructed from immutable versions.

## Operational risks

Address licensing, timestamp ambiguity, delayed delivery, schema drift, partial outages, duplicate records, unit changes, security and silent fallback.
